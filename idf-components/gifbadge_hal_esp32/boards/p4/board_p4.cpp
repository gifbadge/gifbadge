...
static const char *TAG = "esp32::s3::esp32p4";
...
void esp32p4::PmRelease() {
  esp_pm_lock_release(pmLockHandle);
}

hal::config::Config *esp32p4::GetConfig() {
  return _config;
}

void esp32p4::DebugInfo() {
...
OtaError esp32p4::OtaHeaderValidate(uint8_t const *data) {
  ...
  if (new_header_info->chip_id != ESP_CHIP_ID_ESP32P4) {
...
int esp32p4::OtaStatus() {
  return _ota_status;
}

void esp32p4::OtaInstallTask(void *arg) {
  esp_err_t err;
  auto *board = static_cast<esp32p4 *>(arg);
...
size_t esp32p4::MemorySize() {
  return esp_psram_get_size();
}
esp_err_t esp32p4::esp32p4_usb_init(gpio_num_t usb_sense) {
#if CFG_TUD_CDC
...
void tud_dfu_download_cb(uint8_t alt, uint16_t block_num, uint8_t const *data, uint16_t length) {
  ...
  if (block_num == 0 && length >= OTA_HEADER_SIZE) {
    ...
    Boards::OtaError validation_err = Boards::esp32::p4::esp32p4::OtaHeaderValidate(data);
