...
static const char *TAG = "board_esp32p4_sdmmc";

#define USB_DISABLED

namespace Boards {

StorageInfo esp32::p4::esp32p4_sdmmc::GetStorageInfo() {
...
void esp32::p4::esp32p4_sdmmc::Reset() {
...
void esp32::p4::esp32p4_sdmmc::PowerOff() {
  if(tinyusb_msc_delete_storage(storage_handle) != ESP_OK){
    LOGI(TAG, "Failed to unmount");
  }
}

int esp32::p4::esp32p4_sdmmc::StorageFormat() {
...
const char *esp32::p4::esp32p4_sdmmc::GetStoragePath() {
  return "/data";
}
...
esp_err_t esp32::p4::esp32p4_sdmmc::mount(const gpio_num_t clk,
  ...
  // host.flags |= SDMMC_HOST_FLAG_DDR;
  // host.input_delay_phase = SDMMC_DELAY_PHASE_AUTO;
...
// };
  ...
  if (init_sdmmc_slot(&host, &slot_config, &card) == ESP_OK) {
    ...
    ESP_ERROR_CHECK(esp32p4_usb_init(usb_sense));
...
bool esp32::p4::esp32p4_sdmmc::UsbConnected() {
  if(!storageAvailable){
    return false;
  }
#ifndef USB_DISABLED
...
int esp32::p4::esp32p4_sdmmc::UsbCallBack(tusb_msc_callback_t callback) {
