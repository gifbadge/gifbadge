...
static void StorageFormatOK(lv_event_t *e){
    ...
    fields->storage_menu = static_cast<lv_obj_t *>(lv_obj_get_user_data(static_cast<lv_obj_t *>(lv_event_get_user_data(e))));
...
static void StorageRefresh(lv_event_t *e){
    ...
    lv_bar_set_value(fields->space_bar, ceil(100.00-((static_cast<double>(info.free_bytes)/info.total_bytes)*100)), LV_ANIM_OFF);
    lv_label_set_text_fmt(fields->space_label, "%.0f/%.0fMB", (double)(info.total_bytes-info.free_bytes)/1000/1000, (double)(info.total_bytes)/1000/1000);
}
