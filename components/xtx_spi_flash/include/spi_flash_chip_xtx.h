 ...
 * ``spi_flash_chip_xtx_write`` recursively until the whole page is
 * programmed. Strongly suggest to call ``spi_flash_chip_xtx_write``
 * instead.
 *
 * @param chip Pointer to SPI flash chip to use. If NULL, esp_flash_default_chip is substituted.
 ...
 */
//esp_err_t
//spi_flash_chip_xtx_page_program(esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length);
 ...
 */
//esp_err_t
//spi_flash_chip_xtx_write_encrypted(esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length);
 ...
 *      - or other error passed from the ``common_command`` function of host driver
  */
esp_err_t spi_flash_chip_xtx_get_io_mode(esp_flash_t *chip, esp_flash_io_mode_t* out_quad_mode);
 ...
 */
extern const spi_flash_chip_t esp_flash_chip_xtx;
...
extern const flash_chip_op_timeout_t spi_flash_chip_xtx_timeout;
