...
const DRAM_ATTR flash_chip_op_timeout_t spi_flash_chip_xtx_timeout = {
...
esp_err_t spi_flash_chip_xtx_detect_size(esp_flash_t *chip, uint32_t *size)
{
...
esp_err_t spi_flash_chip_xtx_probe(esp_flash_t *chip, uint32_t flash_id)
{
    const uint8_t MFG_ID = 0x0b;
    if (((flash_id >> 8) & 0x00FF) != MFG_ID) {
        return ESP_ERR_NOT_FOUND;
    }

    uint32_t product_id = flash_id & 0x0000FF;
    if (product_id != 0x11) {
        return ESP_ERR_NOT_FOUND;
    }
    return ESP_OK;
...
esp_err_t spi_flash_chip_xtx_reset(esp_flash_t *chip)
{
...
esp_err_t spi_flash_chip_xtx_erase_chip(esp_flash_t *chip)
{
    esp_err_t err;
...
esp_err_t spi_flash_chip_xtx_erase_sector(esp_flash_t *chip, uint32_t start_address)
{
...
esp_err_t spi_flash_chip_xtx_erase_block(esp_flash_t *chip, uint32_t start_address)
{
    esp_err_t err = chip->chip_drv->set_chip_write_protect(chip, false);
    if (err == ESP_OK) {
        err = chip->chip_drv->wait_idle(chip, chip->chip_drv->timeout->idle_timeout);
...
//esp_err_t spi_flash_chip_xtx_read(esp_flash_t *chip, void *buffer, uint32_t address, uint32_t length)
//{
...
//esp_err_t spi_flash_chip_xtx_page_program(esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length)
//{
//    esp_err_t err;
//
//    err = chip->chip_drv->wait_idle(chip, chip->chip_drv->timeout->idle_timeout);
...
//esp_err_t spi_flash_chip_xtx_write(esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length)
//{
//    esp_err_t err = ESP_OK;
//    const uint32_t page_size = chip->chip_drv->page_size;
//    uint32_t align_address;
...
//esp_err_t spi_flash_chip_xtx_write_encrypted(esp_flash_t *chip, const void *buffer, uint32_t address, uint32_t length)
//{
...
esp_err_t spi_flash_chip_xtx_set_write_protect(esp_flash_t *chip, bool write_protect)
{
    esp_err_t err = ESP_OK;
...
esp_err_t spi_flash_chip_xtx_get_write_protect(esp_flash_t *chip, bool *out_write_protect)
{
    esp_err_t err = ESP_OK;
    uint32_t status;
...
//esp_err_t spi_flash_chip_xtx_read_reg(esp_flash_t* chip, spi_flash_register_t reg_id, uint32_t* out_reg)
//{
//    return chip->host->driver->read_status(chip->host, (uint8_t*)out_reg);
//}

//esp_err_t spi_flash_chip_xtx_yield(esp_flash_t* chip, uint32_t wip)
//{
//    esp_err_t err = ESP_OK;
...
esp_err_t spi_flash_chip_xtx_wait_idle(esp_flash_t *chip, uint32_t timeout_us)
{
...
//esp_err_t spi_flash_chip_xtx_config_host_io_mode(esp_flash_t *chip, uint32_t flags)
//{
...
esp_err_t spi_flash_chip_xtx_get_io_mode(esp_flash_t *chip, esp_flash_io_mode_t* out_io_mode)
{
...
esp_err_t spi_flash_chip_xtx_set_io_mode(esp_flash_t *chip)
{
    // On "generic" chips, this involves checking
...
esp_err_t spi_flash_chip_xtx_read_unique_id(esp_flash_t *chip, uint64_t* flash_unique_id)
{
...
esp_err_t spi_flash_chip_xtx_read_unique_id_none(esp_flash_t *chip, uint64_t* flash_unique_id)
{
    // For flash doesn't support read unique id.
    return ESP_ERR_NOT_SUPPORTED;
}

spi_flash_caps_t spi_flash_chip_xtx_get_caps(esp_flash_t *chip)
{
...
static const char chip_name[] = "XTX";

const spi_flash_chip_t esp_flash_chip_xtx = {
    .name = chip_name,
    .timeout = &spi_flash_chip_xtx_timeout,
    .probe = spi_flash_chip_xtx_probe,
    .reset = spi_flash_chip_xtx_reset,
    .detect_size = spi_flash_chip_xtx_detect_size,
    .erase_chip = spi_flash_chip_xtx_erase_chip,
    .erase_sector = spi_flash_chip_xtx_erase_block,
    .erase_block = spi_flash_chip_xtx_erase_block,
    .sector_size = 2 * 1024,
    .block_erase_size = 128 * 1024,

    // TODO: figure out if generic chip-wide protection bits exist across some manufacturers
    .get_chip_write_protect = spi_flash_chip_xtx_get_write_protect,
    .set_chip_write_protect = spi_flash_chip_xtx_set_write_protect,
    ...
    .wait_idle = spi_flash_chip_xtx_wait_idle,
    .set_io_mode = spi_flash_chip_xtx_set_io_mode,
    .get_io_mode = spi_flash_chip_xtx_get_io_mode,

    .read_reg = spi_flash_chip_generic_read_reg,
    .yield = spi_flash_chip_generic_yield,
    .sus_setup = spi_flash_chip_generic_suspend_cmd_conf,
    .read_unique_id = spi_flash_chip_generic_read_unique_id,
    .get_chip_caps = spi_flash_chip_xtx_get_caps,
    .config_host_io_mode = spi_flash_chip_generic_config_host_io_mode,
};


esp_err_t spi_flash_chip_xtx_suspend_cmd_conf(esp_flash_t *chip)
{
